...
#if defined(NDEBUG)
# error "Kubacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8bdb65aa99a755f92223b09ee14d2847002cb943049de6807ce04a31070fbb1c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kubacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kubacoin Signed Message:\n";
...
{
    // Kubacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Kubacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Kubacoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Kubacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kubacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kubacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x87ef610b21c9ef375b9847d85ee402f15e6335b822470ab6510a2403ae051dca");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "8/24/2021 - Kuba jede bomby";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c627a61af42d6856b9ca0abd3d2366f446f06e9b0ee26b9336e6f0e18dfede6c16b6563d061cbf5c622d99f9a9db480f725717622eac3aa09f4e07f4ee886411") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1629836926;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085014878;

        if (fTestNet)
        {
            block.nTime    = 1629836884;
            block.nNonce   = 385537409;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1837f51c6a256c13f27c12148a8a97c60370603e4db7aebdd1b77360c1cbd694"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfu, 0xc5, 0xb1, 0xde }; // Kubacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KubacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KubacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KubacoinMiner(CWallet *pwallet)
{
    printf("KubacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kubacoin-miner");
    ...
    try { loop {
        ...
        printf("Running KubacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KubacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KubacoinMiner, pwallet));
}
